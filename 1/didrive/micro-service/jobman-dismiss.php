...
//        throw new \Exception('нет должности');
    ...
    if (isset($_REQUEST['s']) && isset($_REQUEST['jobman']) && \Nyos\nyos::checkSecret($_REQUEST['s'], $_REQUEST['jobman']) !== false) {
    ...
    if (empty($_REQUEST['date']))
        ...
        $s = $db->prepare('SELECT * FROM `mod'.\Nyos\mod\JobDesc::$mod_norms_day.'` WHERE jobman = :jm AND  `date` <= :date ORDER BY date DESC LIMIT 1 );');
        $s->execute( [ 
            ':jm' => $_REQUEST['id'] ,
            ':date' => $_REQUEST['date'] 
                ] );
